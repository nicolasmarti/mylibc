...
%.o: %.c %.h #%.opt
	...
	@time frama-c -cpp-extra-args="-I`frama-c -print-path`/libc" -wp -wp-rte -wp-model Typed -wp-split -wp-par 1 -wp-timeout 10 -wp-proof-trace -wp-prover $(PROVERS) -wp-out $(PROOF_OB_DIR)_$< $< #-then -werror-no-no-unknown -werror -werror-no-external
	@echo "------------------------------------"
	@echo "compilation:"
	@clang -c -o $@ $<
	@echo "------------------------------------"
