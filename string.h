...
/*@
  @
  @predicate string_eq(string_t s1, string_t s2) = 
  @  \exists size_t sz;
  @  string_size(s1, sz) && string_size(s2, sz) &&
  @  \forall index_t i; 0 <= i <= sz ==> s1[i] == s2[i];
  @  
  @predicate string_lt(string_t s1, string_t s2) = 
  @  \exists size_t sz1; \exists size_t sz2;
  @  string_size(s1, sz1) && string_size(s2, sz2) &&
  @  \exists index_t i; 0 <= i <= \min(sz1, sz2) &&
  @  (\forall index_t j; s1[j] == s2[j]) &&
  @  s1[i] < s2[i];
  @  
  @predicate string_gt(string_t s1, string_t s2) = 
  @  \exists size_t sz1; \exists size_t sz2;
  @  string_size(s1, sz1) && string_size(s2, sz2) &&
  @  \exists index_t i; 0 <= i <= \min(sz1, sz2) &&
  @  (\forall index_t j; s1[j] == s2[j]) &&
  ...
  @  
  @lemma l1: 
  @  \forall string_t s1; \forall string_t s2;
